...
class RegisterScreen extends ConsumerStatefulWidget {
  ...
  const RegisterScreen({required this.onNavigateToLogin, super.key});
  ...
  void _handleRegister() async {
    ...
    if (_formKey.currentState!.validate()) {
      ...
      if (success && mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(authState.errorMessage ?? 'Pendaftaran gagal. Silakan coba lagi.'),
            backgroundColor: const Color(0xFFE57373),
            behavior: SnackBarBehavior.floating,
            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
          ),
  ...
  void _handleGoogleRegister() async {
    try {
      ...
      if (success && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text('Pendaftaran dengan Google berhasil!'),
            backgroundColor: const Color(0xFF4CAF50),
            behavior: SnackBarBehavior.floating,
            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
          ),
        );
        // Tidak perlu navigate ke login karena sudah otomatis login
      } else if (!success && mounted) {
        final authState = ref.read(authProvider);
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(authState.errorMessage ?? 'Pendaftaran Google gagal'),
            backgroundColor: const Color(0xFFE57373),
            behavior: SnackBarBehavior.floating,
            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
          ),
